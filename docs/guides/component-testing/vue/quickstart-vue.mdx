...
We are using Vue's Composition API with script setup in this guide, but that is
not required for Cypress Component testing and you can use the Options API as
...
Next, we will learn to mount the `<Stepper />` component with the `cy.mount()`
command!
