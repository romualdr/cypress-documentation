...
- `cy.reload()` requires being chained off of `cy`. - `cy.reload()` requires the
  response to be `content-type: text/html`. - `cy.reload()` requires the
  response code to be `2xx` after following redirects. - `cy.reload()` requires
  the load `load` event to eventually fire.
