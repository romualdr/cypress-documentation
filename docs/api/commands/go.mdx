...
- `cy.go()` requires being chained off of `cy`. - `cy.go()` requires the
  response to be `content-type: text/html`. - `cy.go()` requires the response
  code to be `2xx` after following redirects. - `cy.go()` requires the load
