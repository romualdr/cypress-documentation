...
isolation** which defaults to `on`. This means that after every test, the
current page is reset to `about:blank` and cookies, local storage and session
storage in all domains are cleared before each test. This change is opt-in for
now, but will be standardized in a future major release of Cypress, so
eventually all tests will need to be isolated.
