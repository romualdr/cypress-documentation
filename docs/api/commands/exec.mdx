...
- `cy.exec()` requires being chained off of `cy`. - `cy.exec()` requires the
  executed system command to eventually exit. - `cy.exec()` requires that the
  exit code be `0` when `failOnNonZeroExit` is `true`.
