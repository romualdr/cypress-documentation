...
- `cy.tick()` requires being chained off of `cy`. - `cy.tick()` requires that
  `cy.clock()` be called before it.

### Assertions [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Assertions)

- `cy.tick()` is a utility command. - `cy.tick()` will not run assertions.
