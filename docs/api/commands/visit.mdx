...
flag to `true` in the Cypress config. This will allow you to visit the
cross-origin site without errors. However, to interact with the content on the
cross-origin site, you must use a [`cy.origin()`](/api/commands/origin) block.
